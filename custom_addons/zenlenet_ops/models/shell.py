...
class ZenlenetShell(models.AbstractModel):
    ...
    def apply(self):
        ...
        self.env['zenlenet.flow.task'].sudo().search([('done_at', '!=', False), ('state', 'not in', ('done', 'skipped'))]).write({'state': 'done'})
        self.env['zenlenet.flow'].sudo().search([('state', 'not in', ('done', 'cancel'))])._ensure_default_tasks()
