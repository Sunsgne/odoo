...
class ZenlenetFlow(models.Model):
    ...
    def write(self, vals):
        ...
        watched = {
            'state', 'move', 'service_type', 'sales_user_id', 'allocator_user_id',
            'delivery_user_id', 'service_user_id', 'procurement_user_id',
        }
        if watched & set(vals):
            self._sync_assignee()
    ...
    def _person_for(self, state):
        self.ensure_one()
        return self._person_for_team(team_for_move(self.move, state, self.service_type))
    ...
    def _stamp_actor(self):
        ...
        for record in self:
            field = field_by_team.get(team_for_move(record.move, record.state, record.service_type))
    ...
    def _ensure_role(self, state):
        ...
        raise UserError(f'这一步（{step_label(self.move, state, self.service_type)}）由{label}操作。')
